    ...
    public class SecondThread
    {
        ...
        public void SecondThreadMain(Object obj)
        {
            ...
            while (!connected)
            {
                ...
                {
                    ConsoleHelper.WriteToConsole("2 поток", "Подключение отклонено!");
                ...
                else
                {
                    ...
                    {
                        ConsoleHelper.WriteToConsole("2 поток", "Подключение одобрено!");
            ...
            while (connected)
            {
                ...
                {
                    ConsoleHelper.WriteToConsole("2 поток", "Не удалось распознать кадр!");
                    _sendMessage = BuildREJFrame().Build();
                    _post(_sendMessage);
                    _sendSemaphore.Release();
                    continue;
                ...
                {
                    ConsoleHelper.WriteToConsole("2 поток", "Начало блока данных!");
                    isInBlock = true;
                    _sendMessage = BuildRRFrame().Build();
                    _post(_sendMessage);
                    _sendSemaphore.Release();
                ...
                {
                    ConsoleHelper.WriteToConsole("2 поток", "Конец блока данных!");
                ...
                else if (frame.Control.ToByteArray()[0] == (byte)RequestType.Disconnect)
                {
                    if (isInBlock)
                    {
                        ConsoleHelper.WriteToConsole("2 поток", "Получен запрос на отключение во время получения данных!");

                        DrainBuffer();

                        var receivedString = GetReceivedString();

                        ConsoleHelper.WriteToConsole("2 поток", $"Частично полученные данные: \"{receivedString}\"");
                        _receivedDataBits.Clear();
                    }
                ...
                {
                    ConsoleHelper.WriteToConsole("2 поток", "Получен кадр данных!");
